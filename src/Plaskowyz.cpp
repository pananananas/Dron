...
/*!
 * \file
 * \brief Definicja metody Plaskowyz
 *
 *    Plik zawiera definicję klasy Plaskowyz, która
 *    jest podstawą całego programu. Klasa ta jest dziedziczona
 *    przez kolejne klasy.
 */


bool   Plaskowyz:: ObliczWsp(){
    
    ifstream  Plik_Bryla(PLIK_PLASKOWYZA);
    ofstream  Plik_Przeszkody(TworzNazwePlikuPlaskowyza(IloscPlaskowyzow));
    
    if (!Plik_Bryla.is_open()) {
        cerr << endl
        << " Blad otwarcia do odczytu pliku: " << PLIK_PLASKOWYZA << endl
        << endl;
        return false;
    }
    ...
    if (!Plik_Przeszkody.is_open()) {
        cerr << endl
        << " Blad otwarcia do zapisu pliku: " << PLIK_PLASKOWYZA << endl
        << endl;
        return false;
    }
...
int    Plaskowyz:: SprawdzKolizje(Wektor<3> WspDrona){
    ...
    if (distance <= WspDrona[2])    return 4;
    
    return 0;
}


string Plaskowyz:: TworzNazwePlikuPlaskowyza(int I){
    
    std:: ostringstream StrmWy;
    
    StrmWy << "dat/PlikWlasciwyPlaskowyz" << I << ".dat";
    
    return StrmWy.str();
}


vector<Wektor<3>>  Plaskowyz:: ObrysBryly(){
    
    // Ściągam współrzędne z pliku z linii 12 i 18
    
    string NazwaPliku = TworzNazwePlikuPlaskowyza(IloscPlaskowyzow);
